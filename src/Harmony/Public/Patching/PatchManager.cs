...
namespace HarmonyLib.Public.Patching
{
	...
	public static class PatchManager
	{
		...
		{
			string assemblyName = original.Module.Assembly.GetName().Name;
			if (assemblyName == "0Harmony" || assemblyName == "SethtekSandbox.ModTooling")
			{
				HarmonyLib.Tools.Logger.LogText(Tools.Logger.LogChannel.Error, $"Cannot patch {original.Name} in type {original.DeclaringType}");
				return;
			}
