...
public enum LogLevel
{
    ...
    /// <summary>
    /// A fatal error has occurred, which cannot be recovered
    /// from.
    /// </summary>
    Fatal = 1,
    ...
    /// <summary>
    /// A warning has been produced, but does not necessarily
    /// mean that something wrong has happened.
    /// </summary>
    Warning = 3,

    /// <summary>
    /// An important message that should be displayed to the
    /// user.
    /// </summary>
    Message = 8,
...
{
    /// <summary>
    /// Gets the highest log level when there could potentially
    /// be multiple levels provided.
    /// </summary>
    ...
    public static LogLevel GetHighestLevel(this LogLevel levels)
    {
        ...
        Array.Sort(logLevels);

        foreach (LogLevel logLevel in logLevels)
        {
            if ((levels & logLevel) != LogLevel.None)
            {
                return logLevel;
            }
        }
